...
def get_live(id):
    ...
    data = call_api(api = 'content.play', data = post, token = token)
    if 'err' in data:
        ...
        for md_item in data['playerControl']['liveControl']['mosaic']['items']:
            if md_stream == stream_number:
                ...
                if md_id is not None:
                    ...
                    data = call_api(api = 'hcontent.play', data = post, token = token)
        ...
        if epg[start_ts]['endts'] > int(time.mktime(datetime.now().timetuple()))-10:
            ...
            if channels[channel_id]['adult'] == True:
                ...
                if md == True:
                    ...
                    payload = None
                    data = call_api(api = 'page.content.display', data = {'payload': epg[start_ts]['payload']}, token = token)
